...
async function exportSnapshot(scope: "SELECTION" | "PAGE", endpoint: string, includeHidden: boolean, scale: number) {
  ...
  try {
    ...
    figma.ui.postMessage({ type: "progress", message: "Uploading local snapshot…", value: 88 });
    figma.ui.postMessage({ type: "upload", endpoint: endpoint.replace(/\/$/, ""), manifest, files: [...serialized.files, screenshot], assetCount: serialized.files.length, warnings: serialized.warnings });
